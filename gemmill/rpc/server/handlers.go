...
func makeJSONRPCHandler(funcMap map[string]*RPCFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if rpcFunc == nil {
			WriteRPCResponseHTTP(w, gtypes.NewRPCResponse(request.ID, nil, "RPC method unknown: "+request.Method))
			return
		}
		if rpcFunc.ws {
			WriteRPCResponseHTTP(w, gtypes.NewRPCResponse(request.ID, nil, "RPC method is only for websockets: "+request.Method))
			return
		}
		...
		WriteRPCResponseHTTP(w, gtypes.NewRPCResponse(request.ID, result, ""))
	}
}
...
func jsonParamsToArgsWS(rpcFunc *RPCFunc, params []interface{}, wsCtx gtypes.WSRPCContext) ([]reflect.Value, error) {
	if len(rpcFunc.argNames) != len(params) {
		return nil, errors.New(fmt.Sprintf("Expected %v parameters (%v), got %v (%v)",
...
func makeHTTPHandler(rpcFunc *RPCFunc) func(http.ResponseWriter, *http.Request) {
	...
	if rpcFunc.ws {
		return func(w http.ResponseWriter, r *http.Request) {
			WriteRPCResponseHTTP(w, gtypes.NewRPCResponse("", nil, "This RPC method is only for websockets"))
		}
	}
	// All other endpoints
	return func(w http.ResponseWriter, r *http.Request) {
		log.Debugw("HTTP HANDLER", "req", r)
		args, err := httpParamsToArgs(rpcFunc, r)
		if err != nil {
			WriteRPCResponseHTTP(w, gtypes.NewRPCResponse("", nil, fmt.Sprintf("Error converting http params to args: %v", err.Error())))
			return
		}
		returns := rpcFunc.f.Call(args)
		...
		WriteRPCResponseHTTP(w, gtypes.NewRPCResponse("", result, ""))
	}
}
...
type wsConnection struct {
	...
	writeChan   chan gtypes.RPCResponse
...
func NewWSConnection(baseConn *websocket.Conn, funcMap map[string]*RPCFunc, evsw events.EventSwitch) *wsConnection {
	wsc := &wsConnection{
		...
		baseConn:   baseConn,
		writeChan:  make(chan gtypes.RPCResponse, writeChanCapacity), // error when full.
		funcMap:    funcMap,
		evsw:       evsw,
	}
	wsc.BaseService = *gcmn.NewBaseService("wsConnection", wsc)
	return wsc
}
...
// Goroutine-safe
func (wsc *wsConnection) WriteRPCResponse(resp gtypes.RPCResponse) {
	select {
	case <-wsc.Quit:
		return
...
// Nonblocking write.
// Goroutine-safe
func (wsc *wsConnection) TryWriteRPCResponse(resp gtypes.RPCResponse) bool {
	select {
	case <-wsc.Quit:
		return false
	case wsc.writeChan <- resp:
...
func (wsc *wsConnection) readRoutine() {
	...
	for {
		...
		default:
			...
			if err != nil {
				log.Info("Failed to read from connection", zap.String("remote", wsc.remoteAddr), zap.String("err", err.Error()))
			...
			if err != nil {
				...
				wsc.WriteRPCResponse(gtypes.NewRPCResponse(request.ID, nil, errStr))
				continue
			}
			...
			if rpcFunc == nil {
				wsc.WriteRPCResponse(gtypes.NewRPCResponse(request.ID, nil, "RPC method unknown: "+request.Method))
				continue
			}
			var args []reflect.Value
			if rpcFunc.ws {
				wsCtx := gtypes.WSRPCContext{Request: request, WSRPCConnection: wsc}
			...
			if err != nil {
				...
				wsc.WriteRPCResponse(gtypes.NewRPCResponse(request.ID, result, ""))
				continue
			}
...
func (wsc *wsConnection) writeRoutine() {
	...
	for {
		...
		case <-wsc.pingTicker.C:
			...
			if err != nil {
				log.Error("Failed to write ping message on websocket", zap.String("error", err.Error()))
				wsc.Stop()
				return
			}
		case msg := <-wsc.writeChan:
			...
			if err != nil {
				log.Error("Failed to marshal RPCResponse to JSON", zap.String("error", err.Error()))
			} else {
				...
				if err = wsc.baseConn.WriteMessage(websocket.TextMessage, jsonBytes); err != nil {
					log.Warn("Failed to write response on websocket", zap.String("error", err.Error()))
					wsc.Stop()
					return
				}
			}
...
func (wm *WebsocketManager) WebsocketHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Error("Failed to upgrade to websocket connection", zap.String("error", err.Error()))
		return
	}

	// register connection
	con := NewWSConnection(wsConn, wm.funcMap, wm.evsw)
	log.Info("New websocket connection", zap.String("remote", con.remoteAddr))
	con.Start() // Blocking
}
