...
func NewBlockPool(start int64, requestsCh chan<- BlockRequest, timeoutsCh chan<- string) *BlockPool {
	...
	bp.BaseService = *gcmn.NewBaseService("BlockPool", bp)
	return bp
}
...
func (pool *BlockPool) IsCaughtUp() bool {
	...
	if len(pool.peers) == 0 {
		log.Info("Blockpool has no peers")
		return false
	}

	var maxPeerHeight int64 = 0
	for _, peer := range pool.peers {
		maxPeerHeight = gcmn.MaxInt64(maxPeerHeight, peer.height)
	}

	isCaughtUp := (height > 0 || time.Now().Sub(pool.startTime) > 5*time.Second) && (maxPeerHeight == 0 || height >= maxPeerHeight)
	log.Info(gcmn.Fmt("IsCaughtUp: %v", isCaughtUp), zap.Int64("height", height), zap.Int64("maxPeerHeight", maxPeerHeight))
	return isCaughtUp
}
...
func (pool *BlockPool) PopRequest() {
	...
	if r := pool.requesters[pool.height]; r != nil {
		...
		if r := pool.requesters[pool.height]; r == nil || r.block == nil {
			gcmn.PanicSanity("PopRequest() requires a valid block")
		}
		*/
		r.Stop()
		...
		gcmn.PanicSanity(gcmn.Fmt("Expected requester to pop, got nothing at height %v", pool.height))
	}
}
...
func (pool *BlockPool) RedoRequest(height int64) {
	...
	if request.block == nil {
		gcmn.PanicSanity("Expected block to be non-nil")
	}
...
func (pool *BlockPool) debug() string {
	...
	str := ""
	for h := pool.height; h < pool.height+int64(len(pool.requesters)); h++ {
		if pool.requesters[h] == nil {
			str += gcmn.Fmt("H(%v):X ", h)
		} else {
			str += gcmn.Fmt("H(%v):", h)
			str += gcmn.Fmt("B?(%v) ", pool.requesters[h].block != nil)
		}
	}
	return str
}
...
func (peer *bpPeer) onTimeout() {
	...
	log.Error("SendTimeout", zap.String("peer", peer.id), zap.String("reason", "bp onTimeout"))
	peer.didTimeout = true
}
...
func newBPRequester(pool *BlockPool, height int64) *bpRequester {
	...
	bpr.BaseService = *gcmn.NewBaseService("bpRequester", bpr)
	return bpr
}
