...
func (b *Block) TxsIsNil() bool {
	return b.Data == nil || (len(b.Data.Txs) == 0 && len(b.Data.ExTxs) == 0)
}
...
func (b *Block) ValidateBasic(chainID string, lastBlockHeight int64, lastBlockID BlockID,
	...
	if b.ChainID != chainID {
		return errors.New(gcmn.Fmt("Wrong Block.Header.ChainID. Expected %v, got %v", chainID, b.ChainID))
	}
	if b.Height != lastBlockHeight+1 {
		return errors.New(gcmn.Fmt("Wrong Block.Header.Height. Expected %v, got %v", lastBlockHeight+1, b.Height))
	}
	...
	*/
	if b.NumTxs != int64(len(b.Data.Txs)+len(b.Data.ExTxs)) {
		return errors.New(gcmn.Fmt("Wrong Block.Header.NumTxs. Expected %v, got %v", len(b.Data.Txs)+len(b.Data.ExTxs), b.NumTxs))
	}
	...
	if !bytes.Equal(b.AppHash, appHash) {
		return errors.New(gcmn.Fmt("Wrong Block.Header.AppHash.  Expected %X, got %X", appHash, b.AppHash))
	}
...
func (commit *Commit) BitArray() *gcmn.BitArray {
	if commit.bitArray == nil {
		commit.bitArray = gcmn.NewBitArray(len(commit.Precommits))
...
func (data *Data) StringIndented(indent string) string {
	...
	txStrings := make([]string, gcmn.MinInt(len(data.Txs)+len(data.ExTxs), 21))
