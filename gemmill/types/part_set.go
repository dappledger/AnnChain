...
func (part *Part) Hash() []byte {
	if part.hash != nil {
		...
		part.hash = hash.DoHash(part.Bytes)
		return part.hash
	}
}
...
%s}`,
		part.Index,
		indent, gcmn.Fingerprint(part.Bytes),
...
func (psh PartSetHeader) String() string {
	return fmt.Sprintf("%v:%X", psh.Total, gcmn.Fingerprint(psh.Hash))
}
...
func NewPartSetFromData(data []byte, partSize int) *PartSet {
	...
	partsBitArray := gcmn.NewBitArray(total)
	for i := 0; i < total; i++ {
		part := &Part{
			Index: i,
...
func NewPartSetFromHeader(header PartSetHeader) *PartSet {
	return &PartSet{
		...
		partsBitArray: gcmn.NewBitArray(header.Total),
		count:         0,
	}
}
...
func (ps *PartSet) GetReader() io.Reader {
	if !ps.IsComplete() {
		gcmn.PanicSanity("Cannot GetReader() on incomplete PartSet")
	}
