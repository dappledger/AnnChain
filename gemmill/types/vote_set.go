...
func NewVoteSet(chainID string, height int64, round int64, type_ byte, valSet *ValidatorSet) *VoteSet {
	if height == 0 {
		gcmn.PanicSanity("Cannot make VoteSet for height == 0, doesn't make sense.")
	}
	return &VoteSet{
		chainID:       chainID,
		height:        height,
		...
		type_:         type_,
		valSet:        valSet,
		votesBitArray: gcmn.NewBitArray(valSet.Size()),
...
func (voteSet *VoteSet) AddVote(vote *Vote) (added bool, err error) {
	if voteSet == nil {
		gcmn.PanicSanity("AddVote() on nil VoteSet")
	}
	voteSet.mtx.Lock()
...
func (voteSet *VoteSet) addVote(vote *Vote) (added bool, err error) {
	...
	if (vote.Height != voteSet.height) ||
		...
		return false, fmt.Errorf("%v,Expected %d/%d/%d, but got %d/%d/%d",
			ErrVoteUnexpectedStep, voteSet.height, voteSet.round, voteSet.type_,
	...
	if conflicting != nil {
		...
		if !added {
			gcmn.PanicSanity("Expected to add non-conflicting vote")
		}
		return added, nil
	}
...
func (voteSet *VoteSet) addVerifiedVote(vote *Vote, blockKey string, votingPower int64) (added bool, conflicting *Vote) {
	...
	if existing := voteSet.votes[valIndex]; existing != nil {
		if existing.BlockID.Equals(vote.BlockID) {
			gcmn.PanicSanity("addVerifiedVote does not expect duplicate votes")
		} else {
...
func (voteSet *VoteSet) SetPeerMaj23(peerID string, blockID BlockID) {
	if voteSet == nil {
		gcmn.PanicSanity("SetPeerMaj23() on nil VoteSet")
	}
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
...
func (voteSet *VoteSet) GetByAddress(address []byte) *Vote {
	...
	if val == nil {
		gcmn.PanicSanity("GetByAddress(address) returned nil")
	}
	return voteSet.votes[valIndex]
}
...
func (voteSet *VoteSet) MakeCommit() *Commit {
	if voteSet.type_ != VoteTypePrecommit {
		gcmn.PanicSanity("Cannot MakeCommit() unless VoteSet.Type is VoteTypePrecommit")
	}
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
	...
	if voteSet.maj23 == nil {
		gcmn.PanicSanity("Cannot MakeCommit() unless a blockhash has +2/3")
	}
...
type blockVotes struct {
	...
	bitArray  *gcmn.BitArray // valIndex -> hasVote?
...
func newBlockVotes(peerMaj23 bool, numValidators int) *blockVotes {
	return &blockVotes{
		peerMaj23: peerMaj23,
		bitArray:  gcmn.NewBitArray(numValidators),
