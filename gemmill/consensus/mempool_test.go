...
func TestTxConcurrentWithCommit(t *testing.T) {
	...
	deliverTxsRange := func(start, end int) {
		...
		for i := start; i < end; i++ {
			...
			if err != nil {
				panic(gcmn.Fmt("Error after CheckTx: %v", err))
			}
...
func (app *CounterApplication) Info() abci.ResponseInfo {
	return abci.ResponseInfo{Data: gcmn.Fmt("txs:%v", app.txCount)}
}
...
func runTx(tx []byte, countPtr *int) abci.Result {
	...
	if txValue != uint64(count) {
		return abci.ErrBadNonce.AppendLog(gcmn.Fmt("Invalid nonce. Expected %v, got %v", count, txValue))
	}
	*countPtr += 1
	return abci.OK
}
...
func (app *CounterApplication) Query(query []byte) abci.Result {
	return abci.NewResultOK(nil, gcmn.Fmt("Query is not supported"))
}
