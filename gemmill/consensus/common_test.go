...
func fixedConsensusState() *ConsensusState {
	...
	state, _ := sm.MakeStateFromGenesisFile(stateDB, config.GetString("genesis_file"))
...
func randConsensusNet(nValidators int, testName string, tickerFunc func() TimeoutTicker, appFunc func() abci.Application) []*ConsensusState {
	...
	for i := 0; i < nValidators; i++ {
		...
		state := sm.MakeStateFromGenesis(db, genDoc)
		state.Save()
		thisConfig := tendermint_test.ResetConfig(gcmn.Fmt("%s_%d", testName, i))
...
func randGenesisState(numValidators int, randPower bool, minPower int64) (*sm.State, []*types.PrivValidator) {
	...
	db := dbm.NewMemDB()
	s0 := sm.MakeStateFromGenesis(db, genDoc)
	s0.Save()
