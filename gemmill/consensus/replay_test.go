...
func newTestCase(name string, stepChanges []int) *testCase {
	if len(stepChanges) != 3 {
		panic(gcmn.Fmt("a full wal has 3 step changes! Got array %v", stepChanges))
	}
	return &testCase{
		name:    name,
...
func waitForBlock(newBlockCh chan interface{}, thisCase *testCase, i int) {
	...
	case <-newBlockCh:
	case <-after:
		panic(gcmn.Fmt("Timed out waiting for new block for case '%s' line %d", thisCase.name, i))
	}
}
...
func setupReplayTest(thisCase *testCase, nLines int, crashAfter bool) (*ConsensusState, chan interface{}, string, string) {
	fmt.Println("-------------------------------------")
	log.Notice(gcmn.Fmt("Starting replay test %v (of %d lines of WAL). Crash after = %v", thisCase.name, nLines, crashAfter))
