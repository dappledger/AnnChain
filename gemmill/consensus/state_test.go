...
func TestProposerSelection0(t *testing.T) {
	...
	if !bytes.Equal(prop.Address, vss[1].Address) {
		panic(gcmn.Fmt("expected proposer to be validator %d. Got %X", 1, prop.Address))
	}
}
...
func TestProposerSelection2(t *testing.T) {
	...
	for i := 0; i < len(vss); i++ {
		...
		if !bytes.Equal(prop.Address, vss[(i+2)%len(vss)].Address) {
			panic(gcmn.Fmt("expected proposer to be validator %d. Got %X", (i+2)%len(vss), prop.Address))
		}
...
func TestLockNoPOL(t *testing.T) {
	...
	*/
	...
	if !bytes.Equal(rs.ProposalBlock.Hash(), rs.LockedBlock.Hash()) {
		panic(gcmn.Fmt("Expected proposal block to be locked block. Got %v, Expected %v", rs.ProposalBlock, rs.LockedBlock))
	}
