...
type HeightVoteSet struct {
	...
	mtx               sync.Mutex
	round             int64                  // max tracked round
	roundVoteSets     map[int64]RoundVoteSet // keys: [0...round]
	peerCatchupRounds map[string][]int64     // keys: peer.Key; values: at most 2 rounds
}
...
func (hvs *HeightVoteSet) Reset(height int64, valSet *types.ValidatorSet) {
	...
	hvs.height = height
	hvs.valSet = valSet
	hvs.roundVoteSets = make(map[int64]RoundVoteSet)
	hvs.peerCatchupRounds = make(map[string][]int64)
...
func (hvs *HeightVoteSet) Round() int64 {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
	return hvs.round
}
...
func (hvs *HeightVoteSet) SetRound(round int64) {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
	if hvs.round != 0 && (round < hvs.round+1) {
		gcmn.PanicSanity("SetRound() must increment hvs.round")
	}
...
func (hvs *HeightVoteSet) addRound(round int64) {
	if _, ok := hvs.roundVoteSets[round]; ok {
		gcmn.PanicSanity("addRound() for an existing round")
	}
...
func (hvs *HeightVoteSet) Prevotes(round int64) *types.VoteSet {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
	return hvs.getVoteSet(round, types.VoteTypePrevote)
}

func (hvs *HeightVoteSet) Precommits(round int64) *types.VoteSet {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
...
func (hvs *HeightVoteSet) POLInfo() (polRound int64, polBlockID types.BlockID) {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
...
func (hvs *HeightVoteSet) getVoteSet(round int64, type_ byte) *types.VoteSet {
	...
	default:
		gcmn.PanicSanity(gcmn.Fmt("Unexpected vote type %X", type_))
		return nil
	}
}
...
func (hvs *HeightVoteSet) StringIndented(indent string) string {
	...
	for round := int64(0); round <= hvs.round; round++ {
	...
	return gcmn.Fmt(`HeightVoteSet{H:%v R:0~%v
%s  %v
%s}`,
...
func (hvs *HeightVoteSet) SetPeerMaj23(round int64, type_ byte, peerID string, blockID types.BlockID) {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
