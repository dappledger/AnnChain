...
func (conR *ConsensusReactor) Receive(chID byte, src *p2p.Peer, msgBytes []byte) {
	if !conR.IsRunning() {
		log.Debugw("conR not running,Receive", "src", src, "chId", chID, "bytes", msgBytes)
		return
	}

	_, msg, err := DecodeMessage(msgBytes)
	if err != nil {
		log.Warnw("Error decoding message", "src", src, "chId", chID, "msg", msg, "error", err, "bytes", msgBytes)
		// TODO punish peer?
		return
	}
	...
	case DataChannel:
		if conR.fastSync {
			log.Warnw("Ignoring message received during fastSync", "msg", msg)
			return
		}
		switch msg := msg.(type) {
...
func (conR *ConsensusReactor) registerEventCallbacks() {
	...
	types.AddListenerForEvent(conR.evsw, "conR", types.EventStringSwitchToConsensus(), func(data types.TMEventData) {
...
OUTER_LOOP:
	for {
		...
		if !peer.IsRunning() || !conR.IsRunning() {
			log.Infof("Stopping gossipDataRoutine for %v.", peer)
			return
		}
		rs := conR.conS.GetRoundState()
		prs := ps.GetRoundState()
		...
		if (0 < prs.Height) && (prs.Height < rs.Height) {
			if index, ok := prs.ProposalBlockParts.Not().PickRandom(); ok {
				...
				if blockMeta == nil {
					log.Warnw("Failed to load block meta", "peer height", prs.Height, "our height", rs.Height, "blockstore height", conR.conS.blockStore.Height(), "pv", conR.conS.privValidator)
					time.Sleep(peerGossipSleepDuration)
					continue OUTER_LOOP
				} else if !blockMeta.PartsHeader.Equals(prs.ProposalBlockPartsHeader) {
					log.Debugw("Peer ProposalBlockPartsHeader mismatch, sleeping",
				...
				if part == nil {
					log.Warnw("Could not load part", "index", index,
						"peerHeight", prs.Height, "blockPartsHeader", blockMeta.PartsHeader, "peerBlockPartsHeader", prs.ProposalBlockPartsHeader)
					time.Sleep(peerGossipSleepDuration)
					continue OUTER_LOOP
				}
...
OUTER_LOOP:
	for {
		// Manage disconnects from self or peer.
		if !peer.IsRunning() || !conR.IsRunning() {
			log.Infof("Stopping gossipVotesRoutine for %v.", peer)
			return
		}
		rs := conR.conS.GetRoundState()
		prs := ps.GetRoundState()
		...
		if prs.Height != 0 && rs.Height >= prs.Height+2 {
			...
			//log.Infow("Loaded BlockCommit for catch-up", "height", prs.Height, "commit", commit)
		...
		if sleeping == 0 {
			...
			sleeping = 1
			log.Debugw("No votes to send, sleeping", "peer", peer,
...
OUTER_LOOP:
	for {
		// Manage disconnects from self or peer.
		if !peer.IsRunning() || !conR.IsRunning() {
			log.Infof("Stopping queryMaj23Routine for %v.", peer)
			return
		}
...
type PeerRoundState struct {
	...
	Round                    int64               // Round peer is at, -1 if unknown.
	Step                     RoundStepType       // Step peer is at
	StartTime                time.Time           // Estimated start of round 0 at this height
	Proposal                 bool                // True if peer has proposal for this round
	ProposalBlockPartsHeader types.PartSetHeader //
	ProposalBlockParts       *gcmn.BitArray      //
	ProposalPOLRound         int64               // Proposal's POL round. -1 if none.
	ProposalPOL              *gcmn.BitArray      // nil until ProposalPOLMessage received.
	Prevotes                 *gcmn.BitArray      // All votes peer has for this round
	Precommits               *gcmn.BitArray      // All precommits peer has for this round
	LastCommitRound          int64               // Round of commit for last height. -1 if none.
	LastCommit               *gcmn.BitArray      // All commit precommits of commit for last height.
	CatchupCommitRound       int64               // Round that we have commit for. Not necessarily unique. -1 if none.
	CatchupCommit            *gcmn.BitArray      // All commit precommits peer has for this height & CatchupCommitRound
}
...
func (ps *PeerState) SetHasProposal(proposal *types.Proposal) {
	...
	ps.ProposalBlockParts = gcmn.NewBitArray(proposal.BlockPartsHeader.Total)
	ps.ProposalPOLRound = proposal.POLRound
	ps.ProposalPOL = nil // Nil until ProposalPOLMessage received.
}

func (ps *PeerState) SetHasProposalBlockPart(height int64, round int64, index int) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
...
func (ps *PeerState) getVoteBitArray(height, round int64, type_ byte) *gcmn.BitArray {
	if !types.IsVoteTypeValid(type_) {
		gcmn.PanicSanity("Invalid vote type")
	}
...
func (ps *PeerState) ensureCatchupCommitRound(height, round int64, numValidators int) {
	if ps.Height != height {
		return
	}
	/*
		...
		if ps.CatchupCommitRound != -1 && ps.CatchupCommitRound != round {
			gcmn.PanicSanity(gcmn.Fmt("Conflicting CatchupCommitRound. Height: %v, Orig: %v, New: %v", height, ps.CatchupCommitRound, round))
		}
	*/
	if ps.CatchupCommitRound == round {
	...
	if round == ps.Round {
		...
		ps.CatchupCommit = gcmn.NewBitArray(numValidators)
	}
}
...
func (ps *PeerState) ensureVoteBitArrays(height int64, numValidators int) {
	if ps.Height == height {
		if ps.Prevotes == nil {
			ps.Prevotes = gcmn.NewBitArray(numValidators)
		}
		if ps.Precommits == nil {
			ps.Precommits = gcmn.NewBitArray(numValidators)
		}
		if ps.CatchupCommit == nil {
			ps.CatchupCommit = gcmn.NewBitArray(numValidators)
		}
		if ps.ProposalPOL == nil {
			ps.ProposalPOL = gcmn.NewBitArray(numValidators)
		}
	} else if ps.Height == height+1 {
		if ps.LastCommit == nil {
			ps.LastCommit = gcmn.NewBitArray(numValidators)
		}
	}
}
...
func (ps *PeerState) setHasVote(height int64, round int64, type_ byte, index int) {
	//log.Debugw("setHasVote(LastCommit)", "lastCommit", ps.LastCommit, "index", index)
...
func (ps *PeerState) ApplyVoteSetBitsMessage(msg *VoteSetBitsMessage, ourVotes *gcmn.BitArray) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
...
type ProposalPOLMessage struct {
	Height           int64
	ProposalPOLRound int64
	ProposalPOL      *gcmn.BitArray
}
