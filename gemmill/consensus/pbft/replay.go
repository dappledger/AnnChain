...
func (cs *ConsensusState) readReplayMessage(msgBytes []byte, newStepCh chan interface{}) error {
	...
	case types.EventDataRoundState:
		log.Info("Replay: New Step", zap.Int64("height", m.Height), zap.Int64("round", m.Round), zap.String("step", m.Step))
	...
	case msgInfo:
		...
		case *ProposalMessage:
			p := msg.Proposal
			log.Infow("Replay: Proposal", "height", p.Height, "round", p.Round, "header",
				p.BlockPartsHeader, "pol", p.POLRound, "peer", peerKey)
		case *BlockPartMessage:
			log.Info("Replay: BlockPart", zap.Int64("height", msg.Height), zap.Int64("round", msg.Round), zap.String("peer", peerKey))
		case *VoteMessage:
			v := msg.Vote
			log.Infow("Replay: Vote", "height", v.Height, "round", v.Round, "type", v.Type,
				"blockID", v.BlockID, "peer", peerKey)
		}

		cs.handleMsg(m, cs.RoundState)
	case timeoutInfo:
		log.Infow("Replay: Timeout", "height", m.Height, "round", m.Round, "step", m.Step, "dur", m.Duration)
...
func (cs *ConsensusState) CatchupReplay(height int64) error {
...
func (cs *ConsensusState) catchupReplay(csHeight int64) error {
	...
	if found {
		return errors.New(gcmn.Fmt("WAL should not contain height %d.", csHeight+1))
	}
	if gr != nil {
		gr.Close()
	}
	...
	if err == io.EOF {
		log.Warn("Replay: wal.group.Search returned EOF", zap.Int64("height", csHeight))
		return nil
	} else if err != nil {
		return err
	}
	if !found {
		return errors.New(gcmn.Fmt("WAL does not contain height %d.", csHeight))
	}
	defer gr.Close()

	log.Info("Catchup by replaying consensus messages", zap.Int64("height", csHeight))
...
func (pb *playback) replayReset(count int, newStepCh chan interface{}) error {
	...
	log.Infof("Reseting from %d to %d", pb.count, count)
	pb.count = 0
	pb.cs = newCS
