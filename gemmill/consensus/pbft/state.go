...
func (tp *TimeoutParams) Propose(round int64) time.Duration {
...
func (tp *TimeoutParams) Prevote(round int64) time.Duration {
...
func (tp *TimeoutParams) Precommit(round int64) time.Duration {
...
func InitTimeoutParamsFromConfig(conf *viper.Viper) *TimeoutParams {
	return &TimeoutParams{
		Propose0:          int64(conf.GetInt("timeout_propose")),
		ProposeDelta:      int64(conf.GetInt("timeout_propose_delta")),
		Prevote0:          int64(conf.GetInt("timeout_prevote")),
		PrevoteDelta:      int64(conf.GetInt("timeout_prevote_delta")),
		Precommit0:        int64(conf.GetInt("timeout_precommit")),
		PrecommitDelta:    int64(conf.GetInt("timeout_precommit_delta")),
		Commit0:           int64(conf.GetInt("timeout_commit")),
		SkipTimeoutCommit: conf.GetBool("skip_timeout_commit"),
	}
}
...
type ConsensusState struct {
	...
	decideProposal func(height, round int64)
	doPrevote      func(height, round int64)
...
func NewConsensusState(conf *viper.Viper, state *sm.State, blockStore *bc.BlockStore, pool types.TxPool) *ConsensusState {
	cs := &ConsensusState{
		...
		timeoutParams:    InitTimeoutParamsFromConfig(conf),
	...
	cs.BaseService = *gcmn.NewBaseService("ConsensusState", cs)

	walDir := cs.config.GetString("cs_wal_dir")
	err := gcmn.EnsureDir(walDir, 0700)
	if err != nil {
		log.Error("Error ensuring ConsensusState wal dir", zap.String("error", err.Error()))
		return nil
	}
	err = cs.OpenWAL(walDir)
	if err != nil {
		log.Error("Error loading ConsensusState wal", zap.String("error", err.Error()))
		return nil
	}
...
func (cs *ConsensusState) String() string {
	...
	return gcmn.Fmt("ConsensusState") //(H:%v R:%v S:%v", cs.Height, cs.Round, cs.Step)
}
...
func (cs *ConsensusState) OnStart() error {
	...
	if (err == io.EOF || !found) && cs.Step == RoundStepNewHeight {
		log.Warn("Height not found in wal. Writing new height", zap.Int64("height", cs.Height))
	...
	// we need the timeoutRoutine for replay so we don't block on the tick chan.
	...
	if err := cs.catchupReplay(cs.Height); err != nil {
		log.Error("Error on catchup replay", zap.String("error", err.Error()))
...
// May block on send if queue is full.
func (cs *ConsensusState) AddProposalBlockPart(height, round int64, part *types.Part, peerKey string) error {
...
func (cs *ConsensusState) updateRoundStep(round int64, step RoundStepType) {
	cs.Round = round
	cs.Step = step
}
...
func (cs *ConsensusState) scheduleTimeout(duration time.Duration, height, round int64, step RoundStepType) {
...
func (cs *ConsensusState) reconstructLastCommit(state *sm.State) {
	...
	for _, precommit := range seenCommit.Precommits {
		...
		if !added || err != nil {
			gcmn.PanicCrisis(gcmn.Fmt("Failed to reconstruct LastCommit: %v", err))
		}
	}
	if !lastPrecommits.HasTwoThirdsMajority() {
		gcmn.PanicSanity("Failed to reconstruct LastCommit: Does not have +2/3 maj")
	}
...
func (cs *ConsensusState) updateToState(state *sm.State) {
	if cs.CommitRound > -1 && 0 < cs.Height && cs.Height != state.LastBlockHeight {
		gcmn.PanicSanity(gcmn.Fmt("updateToState() expected state height of %v but found %v",
			cs.Height, state.LastBlockHeight))
	}
	if cs.state != nil && cs.state.LastBlockHeight+1 != cs.Height {
		// This might happen when someone else is mutating cs.state.
		// Someone forgot to pass in state.Copy() somewhere?!
		gcmn.PanicSanity(gcmn.Fmt("Inconsistent cs.state.LastBlockHeight+1 %v vs cs.Height %v", cs.state.LastBlockHeight+1, cs.Height))
	}
	...
	if cs.state != nil && (state.LastBlockHeight <= cs.state.LastBlockHeight) {
		log.Debug("Ignoring updateToState()", zap.Int64("newHeight", state.LastBlockHeight+1), zap.Int64("oldHeight", cs.state.LastBlockHeight+1))
		return
	}
	...
	if cs.CommitRound > -1 && cs.Votes != nil {
		if !cs.Votes.Precommits(cs.CommitRound).HasTwoThirdsMajority() {
			gcmn.PanicSanity("updateToState(state) called but last Precommit round didn't have +2/3")
		}
	...
	for {
		if maxSteps > 0 {
			if cs.nSteps >= maxSteps {
				log.Error("reached max steps. exiting receive routine")
				cs.nSteps = 0
				return
			}
		}
...
func (cs *ConsensusState) handleMsg(mi msgInfo, rs RoundState) {
	...
	case *VoteMessage:
		...
		_, err := cs.tryAddVote(msg.Vote, peerKey)
	...
	default:
		log.Error("Unknown msg type", zap.Reflect("type", reflect.TypeOf(msg)))
	}
	if err != nil {
		log.Errorw("Error with msg", "type", reflect.TypeOf(msg), "peer", peerKey, "error", err, "msg", msg)
	}
}

func (cs *ConsensusState) handleTimeout(ti timeoutInfo, rs RoundState) {
	log.Debugw("Received tock", "timeout", ti.Duration, "height", ti.Height, "round", ti.Round, "step", ti.Step)

	// timeouts must be for current height, round, step
	if ti.Height != rs.Height || ti.Round < rs.Round || (ti.Round == rs.Round && ti.Step < rs.Step) {
		log.Debugw("Ignoring tock because we're ahead", "height", rs.Height, "round", rs.Round, "step", rs.Step)
		return
	}
	...
	default:
		panic(gcmn.Fmt("Invalid timeout step: %v", ti.Step))
	}
...
func (cs *ConsensusState) enterNewRound(height int64, round int64) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && cs.Step != RoundStepNewHeight) {
		log.Debugf("enterNewRound(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
		return
	}

	if now := time.Now(); cs.StartTime.After(now) {
		log.Info("Need to set a buffer and log.Warn() here for sanity.", zap.Time("startTime", cs.StartTime), zap.Time("now", now))
	}
	// cs.stopTimer()

	log.Infof("enterNewRound(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
...
func (cs *ConsensusState) enterPropose(height int64, round int64) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && RoundStepPropose <= cs.Step) {
		log.Debugf("enterPropose(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
		return
	}
	log.Infof("enterPropose(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
	...
	if !bytes.Equal(cs.Validators.Proposer().Address, cs.privValidator.GetAddress()) {
		log.Infow("enterPropose: Not our turn to propose", "proposer", fmt.Sprintf("%X", cs.Validators.Proposer().Address), "privValidator", cs.privValidator)
	} else {
		log.Infow("enterPropose: Our turn to propose", "proposer", fmt.Sprintf("%X", cs.Validators.Proposer().Address), "privValidator", cs.privValidator)
		cs.decideProposal(height, round)
	}
}

func (cs *ConsensusState) defaultDecideProposal(height, round int64) {
	...
	if err == nil {
		...
		*/
		...
		log.Infof("Signed proposal height %d round %d proposal %v", height, round, proposal)
		//log.Debugf("Signed proposal block: %v", block)
	} else {
		if !cs.replayMode {
			log.Error("enterPropose: Error signing proposal", zap.Int64("height", height), zap.Int64("round", round), zap.String("error", err.Error()))
		}
	}
}
...
func (cs *ConsensusState) createProposalBlock() (block *types.Block, blockParts *types.PartSet) {
	...
	alltxs := cs.mempool.Reap(cs.config.GetInt("block_size"))
	extxs := []types.Tx{}
	txs := []types.Tx{}
...
// Otherwise vote nil.
func (cs *ConsensusState) enterPrevote(height int64, round int64) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && RoundStepPrevote <= cs.Step) {
		log.Debugf("enterPrevote(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
		return
	}
	...
	log.Infof("enterPrevote(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
...
func (cs *ConsensusState) defaultDoPrevote(height int64, round int64) {
	...
	if err != nil {
		...
		log.Error("enterPrevote: ProposalBlock is invalid", zap.String("error", err.Error()))
		cs.signAddVote(types.VoteTypePrevote, nil, types.PartSetHeader{})
		return
	}
...
func (cs *ConsensusState) enterPrevoteWait(height int64, round int64) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && RoundStepPrevoteWait <= cs.Step) {
		log.Debugf("enterPrevoteWait(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
		return
	}
	if !cs.Votes.Prevotes(round).HasTwoThirdsAny() {
		gcmn.PanicSanity(gcmn.Fmt("enterPrevoteWait(%v/%v), but Prevotes does not have any +2/3 votes", height, round))
	}
	log.Infof("enterPrevoteWait(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
...
func (cs *ConsensusState) enterPrecommit(height int64, round int64) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && RoundStepPrecommit <= cs.Step) {
		log.Debugf("enterPrecommit(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
		return
	}
	var precommitedBlock *types.Block
	// cs.stopTimer()

	log.Infof("enterPrecommit(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
	...
	if polRound < round {
		gcmn.PanicSanity(gcmn.Fmt("This POLRound should be %v but got %", round, polRound))
	}
	...
	//	log.Debug("enterPrecommit: proposal block is empty, vote nil, no locking")
	//	cs.signAddVote(types.VoteTypePrecommit, nil, types.PartSetHeader{})
	//	return
	// }

	// if cs.LockedBlock.HashesTo(blockID.Hash) && cs.LockedBlock.NumTxs == 0 && round < emptyRoundLimit {
	//	log.Debug("enterPrecommit: locked block is empty, vote nil, unlocking")
	//	cs.signAddVote(types.VoteTypePrecommit, nil, types.PartSetHeader{})
	//	cs.LockedBlock = nil
	//	cs.LockedRound = 0
	//	cs.LockedBlockParts = nil
	...
	if cs.ProposalBlock.HashesTo(blockID.Hash) {
		log.Debugf("enterPrecommit: +2/3 prevoted proposal block. Locking hash %X", blockID.Hash)
		// Validate the block.
		if err := cs.state.ValidateBlock(cs.ProposalBlock); err != nil {
			gcmn.PanicConsensus(gcmn.Fmt("enterPrecommit: +2/3 prevoted for an invalid block: %v", err))
		}
		cs.LockedRound = round
...
// Enter: any +2/3 precommits for next round.
func (cs *ConsensusState) enterPrecommitWait(height int64, round int64) {
	if cs.Height != height || round < cs.Round || (cs.Round == round && RoundStepPrecommitWait <= cs.Step) {
		log.Debugf("enterPrecommitWait(%v/%v): Invalid args. Current step: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
		return
	}
	if !cs.Votes.Precommits(round).HasTwoThirdsAny() {
		gcmn.PanicSanity(gcmn.Fmt("enterPrecommitWait(%v/%v), but Precommits does not have any +2/3 votes", height, round))
	}
	log.Infof("enterPrecommitWait(%v/%v). Current: %v/%v/%v", height, round, cs.Height, cs.Round, cs.Step)
...
func (cs *ConsensusState) enterCommit(height int64, commitRound int64) {
	if cs.Height != height || RoundStepCommit <= cs.Step {
		log.Debugf("enterCommit(%v/%v): Invalid args. Current step: %v/%v/%v", height, commitRound, cs.Height, cs.Round, cs.Step)
		return
	}
	log.Infof("enterCommit(%v/%v). Current: %v/%v/%v", height, commitRound, cs.Height, cs.Round, cs.Step)
	...
	if !ok {
		gcmn.PanicSanity("RunActionCommit() expects +2/3 precommits")
	}
...
func (cs *ConsensusState) tryFinalizeCommit(height int64) {
	if cs.Height != height {
		gcmn.PanicSanity(gcmn.Fmt("tryFinalizeCommit() cs.Height: %v vs height: %v", cs.Height, height))
	}
	...
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		...
		log.Info(fmt.Sprintf("Attempt to finalize failed. We don't have the commit block. proposal-block: %s , commit-block: %s", cs.ProposalBlock.Hash(), blockID.Hash))
		return
	}
	//	go
...
func (cs *ConsensusState) finalizeCommit(height int64) {
	if cs.Height != height || cs.Step != RoundStepCommit {
		log.Debugf("finalizeCommit(%v): Invalid args. Current step: %v/%v/%v", height, cs.Height, cs.Round, cs.Step)
		return
	}
	// log.Info("ann-stopwatch consensusTime elapsed ", cs.RoundState.CommitTime.Sub(cs.RoundState.StartTime).String())

	blockID, ok := cs.Votes.Precommits(cs.CommitRound).TwoThirdsMajority()
	block, blockParts := cs.ProposalBlock, cs.ProposalBlockParts

	if !ok {
		gcmn.PanicSanity(gcmn.Fmt("Cannot finalizeCommit, commit does not have two thirds majority"))
	}
	if !blockParts.HasHeader(blockID.PartsHeader) {
		gcmn.PanicSanity(gcmn.Fmt("Expected ProposalBlockParts header to be commit header"))
	}
	if !block.HashesTo(blockID.Hash) {
		gcmn.PanicSanity(gcmn.Fmt("Cannot finalizeCommit, ProposalBlock does not hash to commit hash"))
	}
	if err := cs.state.ValidateBlock(block); err != nil {
		gcmn.PanicConsensus(gcmn.Fmt("+2/3 committed an invalid block: %v", err))
	}

	log.Infof("Finalizing commit of block with %d txs, height %d, hash %X, root %x", block.NumTxs, block.Height, block.Hash(), block.AppHash)
	//log.Debugf("%v", block)
	...
	if cs.blockStore.Height() < block.Height {
		...
		log.Warn("Why are we finalizeCommitting a block height we already have?", zap.Int64("height", block.Height))
	}
	...
	if err != nil {
		// TODO!
		log.Error("apply block", zap.Error(err))
	}
...
func (cs *ConsensusState) addProposalBlockPart(height int64, part *types.Part, verify bool) (added bool, err error) {
	...
	if added && cs.ProposalBlockParts.IsComplete() {
		...
		//log.Debug("Received complete proposal block", zap.Int64("height", cs.ProposalBlock.Height), zap.String("hash", gcmn.Fmt("%X", cs.ProposalBlock.Hash())))
...
func (cs *ConsensusState) tryAddVote(vote *types.Vote, peerKey string) (bool, error) {
	added, err := cs.addVote(vote, peerKey)
	if err != nil {
		...
		if err == ErrVoteHeightMismatch {
			...
			if peerKey == "" {
				log.Warn("Found conflicting vote from ourselves. Did you unsafe_reset a validator?", zap.Int64("height", vote.Height), zap.Int64("round", vote.Round), zap.Binary("type", []byte{vote.Type}))
				return added, err
			}
			...
			log.Error("Error attempting to add vote", zap.String("error", err.Error()))
...
func (cs *ConsensusState) addVote(vote *types.Vote, peerKey string) (added bool, err error) {
	// log.Debug("addVote", zap.Int64("voteHeight", vote.Height), zap.Binary("voteType", []byte{vote.Type}), zap.Int64("csHeight", cs.Height))
	...
	if vote.Height+1 == cs.Height {
		...
		if added {
			log.Debug("Added to lastPrecommits: " + cs.LastCommit.StringShort())
	...
	if vote.Height == cs.Height {
		...
		if added {
			...
			case types.VoteTypePrevote:
				...
				log.Debugw("Added to prevote", "vote", vote, "prevotes", prevotes.StringShort())
				...
				if (cs.LockedBlock != nil) && (cs.LockedRound < vote.Round) && (vote.Round <= cs.Round) {
					...
					if ok && !cs.LockedBlock.HashesTo(blockID.Hash) {
						log.Info("Unlocking because of POL.", zap.Int64("lockedRound", cs.LockedRound), zap.Int64("POLRound", vote.Round))
						cs.LockedRound = 0
						cs.LockedBlock = nil
						cs.LockedBlockParts = nil
						types.FireEventUnlock(cs.evsw, cs.RoundStateEvent())
			...
			case types.VoteTypePrecommit:
				...
				log.Debugw("Added to precommit", "vote", vote, "precommits", precommits.StringShort())
			...
			default:
				gcmn.PanicSanity(gcmn.Fmt("Unexpected vote type %X", vote.Type)) // Should not happen.
			}
		}
	...
	log.Debug("Vote ignored and not added", zap.Int64("voteHeight", vote.Height), zap.Int64("csHeight", cs.Height), zap.String("err", err.Error()))
	return
}
...
func (cs *ConsensusState) signAddVote(type_ byte, hash []byte, header types.PartSetHeader) *types.Vote {
	...
	if err == nil {
		...
		//log.Debugf("Signed and pushed vote height %d, round %d, vote %v, error %v", cs.Height, cs.Round, vote, err)
		return vote
	} else {
		if !cs.replayMode {
			log.Warnf("Error signing vote height %d, round %d, vote %v, error %v", cs.Height, cs.Round, vote, err)
		}
		return nil
	}
}
...
func CompareHRS(h1, r1 int64, s1 RoundStepType, h2, r2 int64, s2 RoundStepType) int64 {
	if h1 < h2 {
		return -1
	} else if h1 > h2 {
		return 1
