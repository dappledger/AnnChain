...
func NewRouter(config *viper.Viper, state *sm.State, priv *agtypes.PrivValidator) *Router {
	...
	router.BaseService = *cmn.NewBaseService("TraceRouter", router)
	return router
}
...
func (rt *Router) request(requestHash []byte, fromPeer string, data []byte, onlyValidator bool) (broadcast bool, err error) {
