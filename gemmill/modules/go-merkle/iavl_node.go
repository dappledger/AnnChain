...
func (node *IAVLNode) _copy() *IAVLNode {
	if node.height == 0 {
		gcmn.PanicSanity("Why are you copying a value node?")
	}
	return &IAVLNode{
		key:       node.key,
		height:    node.height,
...
func (node *IAVLNode) getByIndex(t *IAVLTree, index int) (key []byte, value []byte) {
	if node.height == 0 {
		if index == 0 {
			...
			gcmn.PanicSanity("getByIndex asked for invalid index")
			return nil, nil
		}
	} else {
...
func (node *IAVLNode) hashWithCount(t *IAVLTree) ([]byte, int) {
	...
	node.hash = hash.DoHash(buf.Bytes())
...
func (node *IAVLNode) writeHashBytes(t *IAVLTree, w io.Writer) (n int, hashCount int, err error) {
	...
	if node.height == 0 {
		...
		if node.leftHash == nil {
			gcmn.PanicSanity("node.leftHash was nil in writeHashBytes")
		}
		...
		if node.rightHash == nil {
			gcmn.PanicSanity("node.rightHash was nil in writeHashBytes")
		}
...
func (node *IAVLNode) writePersistBytes(t *IAVLTree, w io.Writer) (n int, err error) {
	...
	if node.height == 0 {
		...
		// left
		if node.leftHash == nil {
			gcmn.PanicSanity("node.leftHash was nil in writePersistBytes")
		}
		wire.WriteByteSlice(node.leftHash, w, &n, &err)
		// right
		if node.rightHash == nil {
			gcmn.PanicSanity("node.rightHash was nil in writePersistBytes")
		}
		wire.WriteByteSlice(node.rightHash, w, &n, &err)
	}
	return
