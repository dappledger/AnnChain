...
func (t *IAVLTree) Copy() Tree {
	...
	if t.ndb != nil && !t.root.persisted {
		...
		gcmn.PanicSanity("It is unsafe to Copy() an unpersisted tree.")
...
func (ndb *nodeDB) GetNode(t *IAVLTree, hash []byte) *IAVLNode {
	...
	if ok {
		...
		if len(buf) == 0 {
			// ndb.db.Print()
			gcmn.PanicSanity(gcmn.Fmt("Value missing for key %X", hash))
		}
		node, err := MakeIAVLNode(buf, t)
		if err != nil {
			gcmn.PanicCrisis(gcmn.Fmt("Error reading IAVLNode. bytes: %X  error: %v", buf, err))
		}
		node.hash = hash
		node.persisted = true
		ndb.cacheNode(node)
...
func (ndb *nodeDB) SaveNode(t *IAVLTree, node *IAVLNode) {
	...
	defer ndb.mtx.Unlock()
	if node.hash == nil {
		gcmn.PanicSanity("Expected to find node.hash, but none found.")
	}
	if node.persisted {
		gcmn.PanicSanity("Shouldn't be calling save on an already persisted node.")
	}
...
func (ndb *nodeDB) RemoveNode(t *IAVLTree, node *IAVLNode) {
	...
	if !node.persisted {
		gcmn.PanicSanity("Shouldn't be calling remove on a non-persisted node.")
	}
