...
func OpenGroup(headPath string) (g *Group, err error) {
	...
	g.BaseService = *gcmn.NewBaseService("Group", g)
...
func (g *Group) RotateFile() {
	...
	//windows下,rename会报错"xxx used by another process";这里先close,再执行rename.
	//理论上来说,这里lock了，所以，最多一次close就ok了，可以rename了.
	for {
...
func MakeSimpleSearchFunc(prefix string, target int) SearchFunc {
	return func(line string) (int, error) {
		if !strings.HasPrefix(line, prefix) {
			return -1, errors.New(gcmn.Fmt("Marker line did not have prefix: %v", prefix))
		}
		i, err := strconv.Atoi(line[len(prefix):])
		if err != nil {
			return -1, errors.New(gcmn.Fmt("Failed to parse marker line: %v", err.Error()))
		}
		if target < i {
			return 1, nil
		} else if target == i {
