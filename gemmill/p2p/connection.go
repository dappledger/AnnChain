...
type MConnection struct {
	...
	quit         chan struct{}
	flushTimer   *gcmn.ThrottleTimer // flush writes as necessary but throttled.
	pingTimer    *gcmn.RepeatTimer   // send pings periodically
	chStatsTimer *gcmn.RepeatTimer   // update channel stats periodically
...
func NewMConnection(config *viper.Viper, conn net.Conn, chDescs []*ChannelDescriptor, onReceive receiveCbFunc, onError errorCbFunc) *MConnection {
	...
	mconn.BaseService = *gcmn.NewBaseService("MConnection", mconn)
...
func (c *MConnection) OnStart() error {
	...
	c.flushTimer = gcmn.NewThrottleTimer("flush", flushThrottleMS*time.Millisecond)
	c.pingTimer = gcmn.NewRepeatTimer("ping", pingTimeoutSeconds*time.Second)
	c.chStatsTimer = gcmn.NewRepeatTimer("chStats", updateStatsSeconds*time.Second)
	go c.sendRoutine()
	go c.recvRoutine()
	return nil
}
...
func (c *MConnection) flush() {
	//log.Debugw("Flush", "conn", c)
	err := c.bufWriter.Flush()
	if err != nil {
		log.Warn("MConnection flush failed", zap.String("error", err.Error()))
	}
}
...
func (c *MConnection) Send(chID byte, msg interface{}) bool {
	...
	//log.Debugw("Send", "channel", chID, "conn", c, "msg", msg) //, "bytes", wire.BinaryBytes(msg))

	// Send message to channel.
	channel, ok := c.channelsIdx[chID]
	if !ok {
		log.Errorf("Cannot send bytes, unknown channel %X", chID)
		return false
	}
	...
	if success {
		...
		log.Warnw("Send failed", "channel", chID, "conn", c, "msg", msg)
	}
	return success
}
...
func (c *MConnection) TrySend(chID byte, msg interface{}) bool {
	...
	log.Debugw("TrySend", "channel", chID, "conn", c, "msg", msg)
...
func (c *MConnection) CanSend(chID byte) bool {
	...
	channel, ok := c.channelsIdx[chID]
	if !ok {
		log.Errorf("Unknown channel %X", chID)
		return false
	}
	...
	for {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "connection reset by peer") {
				log.Debug("Sleeping " + c.connectResetWait.String() + " because of 'connection reset by peer'")
				time.Sleep(c.connectResetWait)
			} else {
				log.Warnw("Connection failed @ sendRoutine", "conn", c, "error", err)
				c.stopForError(err)
				break FOR_LOOP
			}
		}
...
func (c *MConnection) sendMsgPacket() bool {
	...
	if err != nil {
		log.Warn("Failed to write msgPacket", zap.String("error", err.Error()))
		c.stopForError(err)
		return true
	}
	c.sendMonitor.Update(int(n))
...
FOR_LOOP:
	for {
		...
		/*
			...
			if numBytes := c.bufReader.Buffered(); numBytes > 0 {
				log.Info("Peek connection buffer", "numBytes", numBytes, "bytes", log15.Lazy{func() []byte {
					bytes, err := c.bufReader.Peek(gcmn.MinInt(numBytes, 100))
					if err == nil {
						return bytes
					} else {
		...
		*/
		...
		if err != nil {
			if c.IsRunning() {
				log.Warn("Connection failed @ recvRoutine (reading byte)", zap.String("conn", fmt.Sprintf("%v", c)), zap.Error(err))
				c.stopForError(err)
			}
			break FOR_LOOP
		}
		...
		case packetTypeMsg:
			...
			if err != nil {
				if c.IsRunning() {
					log.Warn("Connection failed @ recvRoutine", zap.String("conn", fmt.Sprintf("%v", c)), zap.Error(err))
					c.stopForError(err)
				}
				break FOR_LOOP
			}
			...
			if !ok || channel == nil {
				gcmn.PanicQ(gcmn.Fmt("Unknown channel %X", pkt.ChannelID))
			}
			...
			if msgBytes != nil {
				//log.Debug("Received bytes", zap.Binary("chID", []byte{pkt.ChannelID}), zap.Binary("msgBytes", msgBytes))
				c.onReceive(pkt.ChannelID, msgBytes)
			}
		default:
			gcmn.PanicSanity(gcmn.Fmt("Unknown message type %X", pktType))
		}
...
func newChannel(conn *MConnection, desc *ChannelDescriptor) *Channel {
	desc.FillDefaults()
	if desc.Priority <= 0 {
		gcmn.PanicSanity("Channel default priority must be a postive integer")
	}
	return &Channel{
		conn:      conn,
		desc:      desc,
...
func (ch *Channel) nextMsgPacket() msgPacket {
	...
	packet.Bytes = ch.sending[:gcmn.MinInt(maxMsgPacketPayloadSize, len(ch.sending))]
	if len(ch.sending) <= maxMsgPacketPayloadSize {
		...
		ch.sending = ch.sending[gcmn.MinInt(maxMsgPacketPayloadSize, len(ch.sending)):]
	}
	return packet
}
...
func (ch *Channel) writeMsgPacketTo(w io.Writer) (n int, err error) {
	...
	//log.Debugw("Write Msg Packet", "conn", ch.conn, "packet", packet)
