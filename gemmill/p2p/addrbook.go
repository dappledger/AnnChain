...
func NewAddrBook(filePath string, routabilityStrict bool) *AddrBook {
	...
	am.init()
	am.BaseService = *gcmn.NewBaseService("AddrBook", am)
	return am
}
...
func (a *AddrBook) AddOurAddress(addr *NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	log.Debugw("Add our address to book", "addr", addr)
...
func (a *AddrBook) AddAddress(addr *NetAddress, src *NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	log.Debugw("Add address to book", "addr", addr, "src", src)
...
func (a *AddrBook) PickAddress(newBias int) *NetAddress {
	...
	if (newCorrelation+oldCorrelation)*a.rand.Float64() < oldCorrelation {
		...
		gcmn.PanicSanity("Should not happen")
	} else {
...
func (a *AddrBook) GetSelection() []*NetAddress {
	...
	numAddresses := gcmn.MaxInt(
		gcmn.MinInt(minGetSelection, len(allAddr)),
		len(allAddr)*getSelectionPercent/100)
	numAddresses = gcmn.MinInt(maxGetSelection, numAddresses)
...
func (a *AddrBook) saveToFile(filePath string) {
	...
	if err != nil {
		log.Error("Failed to save AddrBook to file", zap.String("err", err.Error()))
		return
	}
	err = gcmn.WriteFileAtomic(filePath, jsonBytes, 0644)
	if err != nil {
		log.Error("Failed to save AddrBook to file", zap.String("file", filePath), zap.String("error", err.Error()))
	}
}
...
func (a *AddrBook) loadFromFile(filePath string) bool {
	...
	if err != nil {
		gcmn.PanicCrisis(gcmn.Fmt("Error opening file %s: %v", filePath, err))
	}
	defer r.Close()
	aJSON := &addrBookJSON{}
	...
	if err != nil {
		gcmn.PanicCrisis(gcmn.Fmt("Error reading file %s: %v", filePath, err))
	}
...
out:
	for {
		...
		case <-dumpAddressTicker.C:
			log.Debug("Saving AddrBook to file", zap.Int("size", a.Size()))
...
func (a *AddrBook) addToNewBucket(ka *knownAddress, bucketIdx int) bool {
	// Sanity check
	if ka.isOld() {
		log.Warnf("Cannot add address already in old bucket to a new bucket: %v", ka)
		return false
	}
...
func (a *AddrBook) addToOldBucket(ka *knownAddress, bucketIdx int) bool {
	// Sanity check
	if ka.isNew() {
		log.Warnf("Cannot add new address to old bucket: %v", ka)
		return false
	}
	if len(ka.Buckets) != 0 {
		log.Warnf("Cannot add already old address to another old bucket: %v", ka)
		return false
	}
...
func (a *AddrBook) removeFromBucket(ka *knownAddress, bucketType byte, bucketIdx int) {
	if ka.BucketType != bucketType {
		log.Warnf("Bucket type mismatch: %v", ka)
		return
	}
...
func (a *AddrBook) addAddress(addr, src *NetAddress) {
	if a.routabilityStrict && !addr.Routable() {
		log.Warnf("Cannot add non-routable address %v", addr)
		return
	}
	...
	log.Debugw("Added new address", "address", addr, "total", a.size())
}
...
func (a *AddrBook) expireNew(bucketIdx int) {
	for addrStr, ka := range a.addrNew[bucketIdx] {
		...
		if ka.isBad() {
			log.Infof("expiring bad address %v", addrStr)
...
func (a *AddrBook) moveToOld(ka *knownAddress) {
	// Sanity check
	if ka.isOld() {
		log.Warnf("Cannot promote address that is already old %v", ka)
		return
	}
	if len(ka.Buckets) == 0 {
		log.Warnf("Cannot promote address that isn't in any new buckets %v", ka)
		return
	}
...
func (ka *knownAddress) addBucketRef(bucketIdx int) int {
	for _, bucket := range ka.Buckets {
		if bucket == bucketIdx {
			log.Warnf("Bucket already exists in ka.Buckets: %v", ka)
			return -1
		}
	}
...
func (ka *knownAddress) removeBucketRef(bucketIdx int) int {
	...
	if len(buckets) != len(ka.Buckets)-1 {
		log.Warnf("bucketIdx not found in ka.Buckets: %v", ka)
		return -1
	}
	ka.Buckets = buckets
	return len(ka.Buckets)
