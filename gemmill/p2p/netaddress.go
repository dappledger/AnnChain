...
func NewNetAddress(addr net.Addr) *NetAddress {
	...
	if !ok {
		...
		//may result in undefined behaviour!`, zap.Stringer("addr", addr))
		return NewNetAddressIPPort(net.IP("0.0.0.0"), 0)
		// NOTE: it would be nice to only not panic if we're in testing ...
		// gcmn.PanicSanity(gcmn.Fmt("Only TCPAddrs are supported. Got: %v", addr))
	}
	ip := tcpAddr.IP
...
func (na *NetAddress) Less(other interface{}) bool {
	if o, ok := other.(*NetAddress); ok {
		...
		gcmn.PanicSanity("Cannot compare unequal types")
		return false
	}
}
