...
*
*	endFunc := xlib.CountSlc(xlib.MEMPOOL_TX_QUEST).Begin()
*	xlib.CountSlc(xlib.MEMPOOL_TX_QUEST).Add(1)
*	defer endFunc()
*
*	mem.questMtx.Lock()
...
*--------------------------Or:-----------------------------------------------
*
*endFunc := xlib.CountSlc(xlib.MEMPOOL_TX_SYNCOTHER).Begin()
*xlib.CountSlc(xlib.MEMPOOL_TX_SYNCOTHER).Add(1)
*
...
func (ss *StStatistic) Begin() func() {
	...
	go func() {
		pass := <-bariar
		//utils.Dbgf("[count per time],%v,const,%v,count,%v", ss.name, pass, ss.count)
		ss.mtx.Lock()
		defer ss.mtx.Unlock()
		if ss.maxPass < pass {
			ss.maxPass = pass
