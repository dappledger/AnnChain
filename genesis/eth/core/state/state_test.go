...
var toAddr = ethcmn.BytesToAddress
...
func (s *StateSuite) SetUpTest(c *checker.C) {
	...
	s.state, _ = New(ethcmn.Hash{}, db)
}

func TestNull(t *testing.T) {
	db, _ := ethdb.NewMemDatabase()
	state, _ := New(ethcmn.Hash{}, db)

	address := ethcmn.HexToAddress("0x823140710bf13990e4500136726d8b55")
	state.CreateAccount(address)
	//value := ethcmn.FromHex("0x823140710bf13990e4500136726d8b55")
	var value ethcmn.Hash
	state.SetState(address, ethcmn.Hash{}, value)
	state.Commit(false)
	value, _ = state.GetState(address, ethcmn.Hash{})
	if !ethcmn.EmptyHash(value) {
...
func (s *StateSuite) TestSnapshot(c *checker.C) {
	...
	var storageaddr ethcmn.Hash
	data1 := ethcmn.BytesToHash([]byte{42})
	data2 := ethcmn.BytesToHash([]byte{43})
...
func TestSnapshot2(t *testing.T) {
	...
	data0 := ethcmn.BytesToHash([]byte{17})
	data1 := ethcmn.BytesToHash([]byte{18})
