...
func NewStateSync(root ethcmn.Hash, database trie.DatabaseReader) *StateSync {
	var syncer *trie.TrieSync

	callback := func(leaf []byte, parent ethcmn.Hash) error {
		var obj struct {
			Nonce    uint64
			Balance  *big.Int
			Root     ethcmn.Hash
		...
		syncer.AddRawEntry(ethcmn.BytesToHash(obj.CodeHash), 64, parent)
...
func (s *StateSync) Missing(max int) []ethcmn.Hash {
