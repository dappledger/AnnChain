...
func makeTestState(t *testing.T) (ethdb.Database, ethcmn.Hash, []*testAccount) {
	...
	for i := byte(0); i < 96; i++ {
		obj := state.GetOrNewStateObject(ethcmn.BytesToAddress([]byte{i}))
		acc := &testAccount{address: ethcmn.BytesToAddress([]byte{i})}
...
// account array.
//func checkStateAccounts(t *testing.T, db ethdb.Database, root ethcmn.Hash, accounts []*testAccount) {
...
//func checkStateConsistency(db ethdb.Database, root ethcmn.Hash) error {
...
//	empty := ethcmn.HexToHash("56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421")
//	db, _ := ethdb.NewMemDatabase()
