...
```go
rlp := ethcmn.Encode("doge")
fmt.Printf("%q\n", rlp) // => "\0x83dog"

rlp = ethcmn.Encode([]interface{}{"dog", "cat"})
fmt.Printf("%q\n", rlp) // => "\0xc8\0x83dog\0x83cat"
decoded := ethcmn.Decode(rlp)
...
value := ethcmn.Encode([]interface{}{"one", 2, "three", []interface{}{42}})
...
```go
val := ethcmn.NewEmptyValue().Append(1).Append("2")
...
```go
val := ethcmn.NewValue([]interface{}{1,"2",[]interface{}{3}})
val.Get(0).Uint() // => 1
val.Get(1).Str()  // => "2"
...
```go
val := ethcmn.NewValueFromBytes(rlpData)
val.Get(0).Uint()
```
