...
func NewDelosApp(config cfg.Config, _logger *zap.Logger) *DelosApp {
...
func (app *DelosApp) Start() {
	version.InitNodeInfo("delos")
}

func (app *DelosApp) Stop() {
	app.chainDb.Close()
	app.dataM.Close()
	app.tmpMap.Close()
}

func (app *DelosApp) makeTempHeader(block *at.Block) {
...
func (app *DelosApp) GetAngineHooks() at.Hooks {
	return app.AngineHooks
}

func (app *DelosApp) CompatibleWithAngine() {}

func (app *DelosApp) checkBeforeExecute(stateDup *stateDup, bs []byte) (*types.Transaction, error) {
...
func (app *DelosApp) ValidTx(state *state.StateDB, tx *types.Transaction) at.Result {
...
func (app *DelosApp) ExecuteTx(stateDup *stateDup, bs []byte) (err error) {
	var (
...
func (app *DelosApp) OnNewRound(height, round int, block *at.Block) (interface{}, error) {
...
func (app *DelosApp) OnExecute(height, round int, block *at.Block) (interface{}, error) {
	var (
...
func (app *DelosApp) OnCommit(height, round int, block *at.Block) (interface{}, error) {
	var (
...
func (app *DelosApp) SaveReceipts(stdup *stateDup) []byte {
...
func (app *DelosApp) SaveDBData() error {
	// begin dbtx
...
func (app *DelosApp) LoadLastBlock() (lastBlock LastBlockInfo) {
...
func (app *DelosApp) SaveLastBlock(appHash []byte, header *types.AppHeader) {
...
func (app *DelosApp) CheckTx(bs []byte) at.Result {
...
func (app *DelosApp) checkEnoughFee(from ethcmn.Address, tx *types.Transaction) bool {
...
// query Info
func (app *DelosApp) Info() (resInfo at.ResultInfo) {
...
func (app *DelosApp) QueryNonce(address string) at.Result {
...
// query accout info
func (app *DelosApp) QueryAccount(address string) at.NewRPCResult {
...
func (app *DelosApp) QueryLedgers(order string, limit uint64, cursor uint64) at.NewRPCResult {
	return app.queryAllLedgers(cursor, limit, order)
}

// query ledger info
func (app *DelosApp) QueryLedger(height uint64) at.NewRPCResult {
	sequence := new(big.Int).SetUint64(height)
	return app.queryLedger(sequence)
}

// query all payments
func (app *DelosApp) QueryPayments(order string, limit uint64, cursor uint64) at.NewRPCResult {
...
func (app *DelosApp) QueryAccountPayments(address string, order string, limit uint64, cursor uint64) at.NewRPCResult {
	if !ethcmn.IsHexAddress(address) {
		return at.NewRpcError(at.CodeType_BaseInvalidInput, "Invalid address")
	}
	if strings.Index(address, "0x") == 0 {
...
func (app *DelosApp) QueryPayment(txhash string) at.NewRPCResult {
	var query types.ActionsQuery
...
func (app *DelosApp) QueryTransactions(order string, limit uint64, cursor uint64) at.NewRPCResult {
	//	var query types.ActionsQuery
	return app.queryAllTxs(cursor, limit, order)
}

// query transaction with txhash
func (app *DelosApp) QueryTransaction(txhash string) at.NewRPCResult {
	if txhash == "" {
		return at.NewRpcError(at.CodeType_BaseInvalidInput, "Invalid txhash")
	}
...
func (app *DelosApp) QueryAccountTransactions(address string, order string, limit uint64, cursor uint64) at.NewRPCResult {
	if !ethcmn.IsHexAddress(address) {
		return at.NewRpcError(at.CodeType_BaseInvalidInput, "Invalid address")
	}
	if strings.Index(address, "0x") == 0 {
...
func (app *DelosApp) QueryLedgerTransactions(height uint64, order string, limit uint64, cursor uint64) at.NewRPCResult {
	heightStr := strconv.FormatUint(height, 10)
	return app.queryHeightTxs(heightStr, cursor, limit, order)
}

// query contract
func (app *DelosApp) QueryDoContract(query []byte) at.NewRPCResult {
	return app.queryDoContract(query)
}

// query contract is exist
func (app *DelosApp) QueryContractExist(address string) at.NewRPCResult {
...
func (app *DelosApp) QueryReceipt(txhash string) at.NewRPCResult {
	hash := ethcmn.HexToHash(txhash)
	if len(hash) != ethcmn.HashLength {
		return at.NewRpcError(at.CodeType_BaseInvalidInput, "Invalid txhash")
	}
...
func (app *DelosApp) QueryAccountManagedatas(address string, order string, limit uint64, cursor uint64) at.NewRPCResult {
	if !ethcmn.IsHexAddress(address) {
		return at.NewRpcError(at.CodeType_BaseInvalidInput, "Invalid address")
	}
	if strings.Index(address, "0x") == 0 {
...
func (app *DelosApp) QueryAccountManagedata(address string, key string) at.NewRPCResult {
	if !ethcmn.IsHexAddress(address) {
		return at.NewRpcError(at.CodeType_BaseInvalidInput, "Invalid address")
	}
	if strings.Index(address, "0x") == 0 {
...
func (app *DelosApp) ParseBaseFee(block *at.Block) *big.Int {
...
func (app *DelosApp) ParseBaseReserve(block *at.Block) *big.Int {
...
func (app *DelosApp) ParseMaxTxSetSize(block *at.Block) uint64 {
	maxTxSetSize := app.config.GetInt("max_txset_size")

	return uint64(maxTxSetSize)
}

func (app *DelosApp) makeCurrentHeader(block *at.Block) *ethtypes.Header {
