...
${ANNPATH}/anntool --callmode="commit" --backend="tcp://127.0.0.1:16657" --target="${CHAINID}" organization create --genesisfile ./shards/evm1/genesis1.json --configfile ./shards/evm1/config1.json --privkey "${PRIV1}"
...
${ANNPATH}/anntool --callmode="commit" --backend="tcp://127.0.0.1:26657" --target="${CHAINID}" organization join --configfile ./shards/evm1/config2.json --privkey="${PRIV2}" --orgid="${ORGID}"
...
${ANNPATH}/anntool --callmode="commit" --backend="tcp://127.0.0.1:36657" --target="${CHAINID}" organization join --configfile ./shards/evm1/config3.json --privkey="${PRIV3}" --orgid="${ORGID}"
...
${ANNPATH}/anntool --callmode="commit" --backend="tcp://127.0.0.1:36657" --target="${CHAINID}" organization join --configfile ./shards/evm1/config4.json --privkey="${PRIV4}"  --orgid="${ORGID}"
