...
const (
	...
	// ReceiptStatusFailedEVMOutOfGas is the status code of a transaction if execution failed because of evm run out of gas.
	ReceiptStatusFailedEVMOutOfGas = uint64(2)
)
...
func (r *Receipt) setStatus(postStateOrStatus []byte) error {
	...
	case bytes.Equal(postStateOrStatus, receiptStatusFailedRLP):
		r.Status = ReceiptStatusFailed
	case bytes.Equal(postStateOrStatus, receiptStatusFailedEVMOutOfGasRLP):
		r.Status = ReceiptStatusFailedEVMOutOfGas
...
func (r *Receipt) statusEncoding() []byte {
	if len(r.PostState) == 0 {
		...
		case ReceiptStatusFailedEVMOutOfGas:
			return receiptStatusFailedEVMOutOfGasRLP
		default:
