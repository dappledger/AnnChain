...
func NewBaseService(log *zap.Logger, name string, impl Service) *BaseService {
	return &BaseService{
		log:  log,
		name: name,
...
func (bs *BaseService) Start() (bool, error) {
	if atomic.CompareAndSwapUint32(&bs.started, 0, 1) {
		if atomic.LoadUint32(&bs.stopped) == 1 {
			if bs.log != nil {
				bs.log.Warn(Fmt("Not starting %v -- already stopped", bs.name))
			}
			return false, nil
		} else {
			if bs.log != nil {
				bs.log.Info(Fmt("Starting %v", bs.name))
			}
		}
		err := bs.impl.OnStart()
		return true, err
		...
		if bs.log != nil {
			bs.log.Debug(Fmt("Not starting %v -- already started", bs.name))
		}
		return false, nil
	}
}
...
func (bs *BaseService) Stop() bool {
	if atomic.CompareAndSwapUint32(&bs.stopped, 0, 1) {
		if bs.log != nil {
			bs.log.Info(Fmt("Stopping %v", bs.name))
		}
		bs.Quit <- struct{}{}
		bs.impl.OnStop()
		close(bs.Quit)
		...
		if bs.log != nil {
			bs.log.Debug(Fmt("Stopping %v (ignoring: already stopped)", bs.name))
		}
		return false
	}
}
...
func (bs *BaseService) Reset() (bool, error) {
	if atomic.CompareAndSwapUint32(&bs.stopped, 1, 0) {
		...
		if bs.log != nil {
			bs.log.Debug(Fmt("Can't reset %v. Not stopped", bs.name))
		}
		return false, nil
	}
	// never happens
...
func NewQuitService(log *zap.Logger, name string, impl Service) *QuitService {
	if log != nil {
