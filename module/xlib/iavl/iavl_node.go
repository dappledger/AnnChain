...
func MakeIAVLNode(buf []byte, t *IAVLTree) (node *IAVLNode, err error) {
	...
	pbn := &pb.IAVLNode{}
	proto.Unmarshal(buf, pbn)

	node = &IAVLNode{}
	node.height = int8(pbn.Height)
	node.size = int(pbn.Size)
	node.key = pbn.Key
	if node.height == 0 {
		node.value = pbn.Value
	} else {
		node.leftHash = pbn.LeftHash
		node.rightHash = pbn.RightHash
	}
	//
	//node = &IAVLNode{}
	//
...
func (node *IAVLNode) writeHashBytes(t *IAVLTree, w io.Writer) (n int, hashCount int, err error) {
	...
	pbn := &pb.IAVLNode{}
	pbn.Height = uint32(node.height)
	pbn.Size = uint64(node.size)

	if node.height == 0 {
		pbn.Key = node.key
		pbn.Value = node.value
	} else {
		// left
		...
		pbn.RightHash = node.rightHash
	}
	bs, err := proto.Marshal(pbn)
	w.Write(bs)
