...
func (t *IAVLTree) Proof(key []byte) (value []byte, proofBytes []byte, exists bool) {
	...
	pbp := &pb.IAVLProof{}
	pbp.LeafHash = proof.LeafHash
	pbp.RootHash = proof.RootHash

	for _, n := range proof.InnerNodes {
		...
		pbp.InnerNodes = append(pbp.InnerNodes, pbpin)
	}
	proofBytes, err := proto.Marshal(pbp)
	if err != nil {
		// panic
	}
	return value, proofBytes, true
