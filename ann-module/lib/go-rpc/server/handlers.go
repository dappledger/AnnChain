...
func RegisterRPCFuncs(logger *zap.Logger, mux *http.ServeMux, funcMap map[string]*RPCFunc) {
	...
	for funcName, rpcFunc := range funcMap {
		mux.HandleFunc("/"+funcName, makeHTTPHandler(logger, rpcFunc))
	}

	// JSONRPC endpoints
	mux.HandleFunc("/", makeJSONRPCHandler(logger, funcMap))
}
...
func makeJSONRPCHandler(logger *zap.Logger, funcMap map[string]*RPCFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			WriteRPCResponseHTTP(w, NewRPCResponse("", nil, -32600, fmt.Sprintf("Error unmarshalling request", err.Error())))
			return
		}
		if len(r.URL.Path) > 1 {
			WriteRPCResponseHTTP(w, NewRPCResponse(request.ID, nil, -32000, fmt.Sprintf("Invalid JSONRPC endpoint %s", r.URL.Path)))
			return
		}
		rpcFunc := funcMap[request.Method]
		if rpcFunc == nil {
			WriteRPCResponseHTTP(w, NewRPCResponse(request.ID, nil, -32601, fmt.Sprintf("RPC method unknown: "+request.Method)))
			return
		}
		if rpcFunc.ws {
			WriteRPCResponseHTTP(w, NewRPCResponse(request.ID, nil, -32001, fmt.Sprintf("RPC method is only for websockets: "+request.Method)))
			return
		}
		args, err := jsonParamsToArgs(rpcFunc, request.Params)
		if err != nil {
			WriteRPCResponseHTTP(w, NewRPCResponse(request.ID, nil, -32700, fmt.Sprintf("Error converting json params to arguments: %v", err.Error())))
			return
		}
		returns := rpcFunc.f.Call(args)
		logger.Sugar().Debugw("HTTPJSONRPC", "method", request.Method, "args", args, "returns", returns)
		result, code, err := unreflectResult(returns)
		if err != nil {
			WriteRPCResponseHTTP(w, NewRPCResponse(request.ID, result, code, err.Error()))
			return
		}
		WriteRPCResponseHTTP(w, NewRPCResponse(request.ID, result, code, ""))
	}
}
...
func makeHTTPHandler(logger *zap.Logger, rpcFunc *RPCFunc) func(http.ResponseWriter, *http.Request) {
	// Exception for websocket endpoints
	if rpcFunc.ws {
		return func(w http.ResponseWriter, r *http.Request) {
			WriteRPCResponseHTTP(w, NewRPCResponse("", nil, -32001, "This RPC method is only for websockets"))
		}
	}
	// All other endpoints
	return func(w http.ResponseWriter, r *http.Request) {
		logger.Sugar().Debugw("HTTP HANDLER", "req", r)
		args, err := httpParamsToArgs(rpcFunc, r)
		if err != nil {
			WriteRPCResponseHTTP(w, NewRPCResponse("", nil, -32701, fmt.Sprintf("Error converting http params to args: %v", err.Error())))
			return
		}
		returns := rpcFunc.f.Call(args)
		logger.Sugar().Debugw("HTTPRestRPC", "method", r.URL.Path, "args", args, "returns", returns)
		result, code, err := unreflectResult(returns)
		if err != nil {
			WriteRPCResponseHTTP(w, NewRPCResponse("", nil, code, err.Error()))
			return
		}
		WriteRPCResponseHTTP(w, NewRPCResponse("", result, code, ""))
	}
}
...
func NewWSConnection(logger *zap.Logger, baseConn *websocket.Conn, funcMap map[string]*RPCFunc, evsw events.EventSwitch) *wsConnection {
	...
	wsc.BaseService = *NewBaseService(logger, "wsConnection", wsc)
	return wsc
}
...
func (wsc *wsConnection) readRoutine() {
	...
	for {
		...
		default:
			...
			if err != nil {
				wsc.logger.Info("Failed to read from connection", zap.String("remote", wsc.remoteAddr), zap.String("err", err.Error()))
			...
			if err != nil {
				...
				wsc.WriteRPCResponse(NewRPCResponse(request.ID, nil, -32600, errStr))
				continue
			}
			...
			if rpcFunc == nil {
				wsc.WriteRPCResponse(NewRPCResponse(request.ID, nil, -32601, fmt.Sprintf("RPC method unknown: "+request.Method)))
				continue
			}
			...
			if err != nil {
				wsc.WriteRPCResponse(NewRPCResponse(request.ID, nil, -32602, err.Error()))
				continue
			}
			returns := rpcFunc.f.Call(args)
			wsc.slogger.Infow("WSJSONRPC", "method", request.Method, "args", args, "returns", returns)
			result, code, err := unreflectResult(returns)
			if err != nil {
				wsc.WriteRPCResponse(NewRPCResponse(request.ID, nil, code, err.Error()))
				continue
			} else {
				wsc.WriteRPCResponse(NewRPCResponse(request.ID, result, code, ""))
				continue
			}
...
func (wsc *wsConnection) writeRoutine() {
	...
	for {
		...
		case <-wsc.pingTicker.C:
			...
			if err != nil {
				wsc.logger.Error("Failed to write ping message on websocket", zap.String("error", err.Error()))
				wsc.Stop()
				return
			}
		case msg := <-wsc.writeChan:
			...
			if err != nil {
				wsc.logger.Error("Failed to marshal RPCResponse to JSON", zap.String("error", err.Error()))
			} else {
				...
				if err = wsc.baseConn.WriteMessage(websocket.TextMessage, jsonBytes); err != nil {
					wsc.logger.Warn("Failed to write response on websocket", zap.String("error", err.Error()))
					wsc.Stop()
					return
				}
			}
...
func (wm *WebsocketManager) WebsocketHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		wm.logger.Error("Failed to upgrade to websocket connection", zap.String("error", err.Error()))
		return
	}

	// register connection
	con := NewWSConnection(wm.logger, wsConn, wm.funcMap, wm.evsw)
	wm.logger.Info("New websocket connection", zap.String("remote", con.remoteAddr))
	con.Start() // Blocking
}
...
func unreflectResult(returns []reflect.Value) (interface{}, int32, error) {
	codeV := returns[1]
	errV := returns[2]
	if errV.Interface() != nil {
		return nil, int32(codeV.Int()), fmt.Errorf("%v", errV.Interface())
	}
	rv := returns[0]
	...
	return rvp.Interface(), int32(codeV.Int()), nil
}
