...
func (a *AddrBook) AddOurAddress(addr *NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	a.slogger.Debugw("Add our address to book", "addr", addr)
...
func (a *AddrBook) AddAddress(addr *NetAddress, src *NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	a.slogger.Debugw("Add address to book", "addr", addr, "src", src)
...
func (a *AddrBook) saveToFile(filePath string) {
	...
	if err != nil {
		a.logger.Error("Failed to save AddrBook to file", zap.String("err", err.Error()))
		return
	}
	err = WriteFileAtomic(filePath, jsonBytes, 0644)
	if err != nil {
		a.logger.Error("Failed to save AddrBook to file", zap.String("file", filePath), zap.String("error", err.Error()))
	}
}
...
out:
	for {
		...
		case <-dumpAddressTicker.C:
			a.logger.Debug("Saving AddrBook to file", zap.Int("size", a.Size()))
...
func (a *AddrBook) addToNewBucket(ka *knownAddress, bucketIdx int) bool {
	// Sanity check
	if ka.isOld() {
		a.slogger.Warnf("Cannot add address already in old bucket to a new bucket: %v", ka)
		return false
	}
...
func (a *AddrBook) addToOldBucket(ka *knownAddress, bucketIdx int) bool {
	// Sanity check
	if ka.isNew() {
		a.slogger.Warnf("Cannot add new address to old bucket: %v", ka)
		return false
	}
	if len(ka.Buckets) != 0 {
		a.slogger.Warnf("Cannot add already old address to another old bucket: %v", ka)
		return false
	}
...
func (a *AddrBook) removeFromBucket(ka *knownAddress, bucketType byte, bucketIdx int) {
	if ka.BucketType != bucketType {
		a.slogger.Warnf("Bucket type mismatch: %v", ka)
		return
	}
...
func (a *AddrBook) addAddress(addr, src *NetAddress) {
	if a.routabilityStrict && !addr.Routable() {
		a.slogger.Warnf("Cannot add non-routable address %v", addr)
		return
	}
	...
	if ka != nil {
		...
		ka = newKnownAddress(a.slogger, addr, src)
	}

	bucket := a.calcNewBucket(addr, src)
	a.addToNewBucket(ka, bucket)

	a.slogger.Debugw("Added new address", "address", addr, "total", a.size())
}
...
func (a *AddrBook) expireNew(bucketIdx int) {
	for addrStr, ka := range a.addrNew[bucketIdx] {
		...
		if ka.isBad() {
			a.slogger.Infof("expiring bad address %v", addrStr)
...
func (a *AddrBook) moveToOld(ka *knownAddress) {
	// Sanity check
	if ka.isOld() {
		a.slogger.Warnf("Cannot promote address that is already old %v", ka)
		return
	}
	if len(ka.Buckets) == 0 {
		a.slogger.Warnf("Cannot promote address that isn't in any new buckets %v", ka)
		return
	}
	...
	if !added {
		...
		if !added {
			...
			if !added {
				a.slogger.Warnf("Could not migrate oldest %v to freedBucket %v", oldest, freedBucket)
			}
		}
		// Finally, add to bucket again.
		added = a.addToOldBucket(ka, oldBucketIdx)
		if !added {
			a.slogger.Warnf("Could not re-add ka %v to oldBucketIdx %v", ka, oldBucketIdx)
		}
	}
}
...
func newKnownAddress(slogger *zap.SugaredLogger, addr *NetAddress, src *NetAddress) *knownAddress {
	return &knownAddress{
		Addr:        addr,
		Src:         src,
		Attempts:    0,
...
func (ka *knownAddress) addBucketRef(bucketIdx int) int {
	for _, bucket := range ka.Buckets {
		if bucket == bucketIdx {
			ka.slogger.Warnf("Bucket already exists in ka.Buckets: %v", ka)
			return -1
		}
	}
...
func (ka *knownAddress) removeBucketRef(bucketIdx int) int {
	...
	if len(buckets) != len(ka.Buckets)-1 {
		ka.slogger.Warnf("bucketIdx not found in ka.Buckets: %v", ka)
		return -1
	}
	ka.Buckets = buckets
	return len(ka.Buckets)
